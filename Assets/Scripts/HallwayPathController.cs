...
public class HallwayPathController : MonoBehaviour {
	...
	void Start () {
		ourNodes = new PathfindingNodeCollection ();
		if (gameObject.CompareTag("Hallway")) 
		{
			for (int nodeNum = -1; nodeNum < 2; nodeNum++) {
				ourNodes.addPathNode(createNewPathNode(0,0,(nodeNum / 2.0f) * transform.localScale.z, pathBoxScale));
			}
		} 
		else if (gameObject.CompareTag("HallwayCorner")) 
		{
			...
			ourNodes.addPathNode(createNewPathNode(-0.5f * transform.localScale.x,0,0, pathBoxScale));
		} 
	...
	Transform createNewPathNode(float x, float y, float z, float boxScale){
